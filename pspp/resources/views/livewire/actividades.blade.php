...
<div class="grid gap-6 mb-3 md:grid-cols-3">
    ...
    <div class="">
        <a href="/detalle-actividad/{{ $item->id }}" class="min-h-12  flex flex-col items-center bg-white border border-gray-200 rounded-lg shadow md:flex-row md:max-w-2xl hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">

            <div class="flex flex-col w-full p-4 leading-normal">

                <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 truncate dark:text-white">{{ $item->nombre }}</h5>
                <p class="mb-3 font-normal text-gray-700 dark:text-gray-400 truncate">{{ $item->descripcion }}</p>
                <div class="flex gap-2 justify-between mb-4">
                    <p class=" font-normal text-gray-700 dark:text-gray-400">
                        <span class="text-gray-300  font-semibold"> Área:</span>
                        {{ $item->area }}
                    </p>
                    <p class=" font-normal text-gray-700 dark:text-gray-400">
                        <span class="text-gray-300 font-semibold">Encargado:</span>
                        {{ $item->encargado }}
                    </p>
                </div>
                <div class="w-full flex justify-end">
                    @if ($item->active)
                    <span class="bg-blue-100 text-blue-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-full dark:bg-blue-900 dark:text-blue-300">En proceso</span>
                    @else
...
<div class="flex items-center p-4 mb-4 text-sm text-yellow-800 rounded-lg bg-yellow-50 dark:bg-gray-800 dark:text-yellow-300" role="alert">
    ...
    <div>
      <span class="font-medium">Falta confirmación!</span> Por favor confirma tu correo para poder continuar.
    </div>
  </div>
@endif
...
<div style="background-color: rgba(0,0,0,0.5)" id="default-modal" tabindex="-1"  class="overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%)] max-h-full">
    @include('livewire.crear-actividad')
</div>
@endif
