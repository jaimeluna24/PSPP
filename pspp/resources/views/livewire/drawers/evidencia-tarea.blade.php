<h5 id="drawer-right-label" class="inline-flex items-center mb-4 text-base font-semibold text-gray-500 dark:text-gray-400"><svg class="w-4 h-4 me-2.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
  ...
  </svg>Evidencia de tareas</h5>
   <button wire:click="toDetalleActividad()" type="button" data-drawer-hide="drawer-right-example" aria-controls="drawer-right-example" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 absolute top-2.5 end-2.5 inline-flex items-center justify-center dark:hover:bg-gray-600 dark:hover:text-white" >
   ...
   <p class="mb-6 text-sm text-gray-500 dark:text-gray-400">
    Aquí encontraras la evidencia que se ha agregado sobre las distintas tareas realizadas a lo largo de la actividad.
    </p>
    @if ($listaEvidenciaTarea && $listaEvidenciaTarea->count() > 0)
    <div class="grid grid-cols-2 md:grid-cols-3 gap-4 mb-3">
        @foreach ($listaEvidenciaTarea as $item)
        <button wire:click="toEvidenciaTarea({{ $item->id }})" data-modal-target="default-modal-evidencia-tarea-image" data-modal-toggle="default-modal-evidencia-tarea-image">
            <img class="h-auto max-w-full rounded-lg" src="{{ asset('storage/' . $item->evidencia) }}" alt="">
        </button>
        @endforeach
            ...
            <div class="grid gap-4 w-60">
                ...
                <div>
                    ...
                    <p
                        ...
                        Por favor agrega alguna evidencia </p>
                </div>
            </div>
        </div>
    </div>
   ...
   <div class="grid grid-cols-2 gap-4">
      <button wire:click="toDetalleActividad()" data-drawer-hide="drawer-right-example" aria-controls="drawer-right-example" class="px-4 py-2 text-sm font-medium text-center text-gray-900 bg-white border border-gray-200 rounded-lg focus:outline-none hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">Cerrar</button>

      @if ($tareaSeleccionada->active && $actividad->active)
      <button wire:click="toAgregarEvidenciaTarea()"
      class="px-4 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Agregar</button>
      @endif
    </div>
